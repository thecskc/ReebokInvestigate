...
class IngestionTime:
    def __init__(self,
                 first_file="./files/product_feed_Reebok-US_20170612.csv"):
    ...
    async def fetch(self, item_urls):
        ...
        if (cur.rowcount != 0):
            ...
            for value in resp:
                print(value["item_url"], self.__convert_date_time_to_string(
                    value["date_created"]))
                return_dict[
                    value["item_url"]] = self.__convert_date_time_to_string(
                    value["date_created"])
    ...
    def append_database_dates(self, filename="batch7.csv"):
        ...
        with open(filename, "w") as csvfile:
            ...
            for key in database_dates_dict.keys():
                if (database_dates_dict.get(
                        key.replace("https", "http")) == "None"):
                    num_not_in_db += 1
                else:
                    str1 = (database_dates_dict.get(key))
                    str1_date_tuple = self.convert_string_to_date(str1)
                    date_1 = date(int(str1_date_tuple[0]),
                                  int(str1_date_tuple[1]),
                                  int(str1_date_tuple[2]))
                    val1 = int(str1)
                    ...
                    date_2 = date(int(str2_date_tuple[0]),
                                  int(str2_date_tuple[1]),
                                  int(str2_date_tuple[2]))
                    val2 = int(str2)
                    num_present += 1
    ...
    def analyze(self, outfile_name="batch7_report.csv",
                image_name="batch7.png"):
        ...
        table.name = "Number of days from obtaining" \
                     " feed file vs Number of items uploaded in database"
